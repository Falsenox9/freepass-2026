...
type IAdminRepository interface {
	CreateCanteenOwner(tx *gorm.DB, user *entity.User, canteen *entity.Canteen) error
	...
	GetCanteenByOwnerID(ownerID uuid.UUID) (*entity.Canteen, error)
...
func (r *AdminRepository) GetCanteenByID(canteenID uuid.UUID) (*entity.Canteen, error) {
	var canteen *entity.Canteen

	err := r.db.Debug().Where("canteen_id = ?", canteenID).First(&canteen).Error
	if err != nil {
		return nil, err
	}
